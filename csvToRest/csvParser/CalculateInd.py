...
def Calculate(path, symbol):
    TechIndicator = pd.read_csv(path + symbol + ".csv")
    # Relative Strength Index
    # Avg(PriceUp)/(Avg(PriceUP)+Avg(PriceDown)*100
    # Where: PriceUp(t)=1*(Price(t)-Price(t-1)){Price(t)- Price(t-1)>0};
    #        PriceDown(t)=-1*(Price(t)-Price(t-1)){Price(t)- Price(t-1)<0};
    TechIndicator.rename(columns={'timestamp':'date'}, inplace=True)

    TechIndicator=TechIndicator.sort_values(by='date', ascending=True) # This now sorts in inversed date order
    print(TechIndicator.head())
    def rsi(values):
    ...
    columns2Drop = [ 'open', 'high', 'low', 'close', 'volume', '26_ema', '12_ema', 'tp', 'rmf', 'pmf', 'nmf', 'mfr']
    TechIndicator = TechIndicator.drop(labels=columns2Drop, axis=1)

    #TechIndicator.index = TechIndicator['date']

    return TechIndicator.sort_values(by='date', ascending=False) # This now sorts in date order


# noinspection DuplicatedCode
def addIndicators(path, symbol):
    print('creating ' + symbol + ' indicators data ...')
    df=Calculate(path=path, symbol=symbol)
    df.to_csv(path + symbol + "Indicators.csv", index=False)
